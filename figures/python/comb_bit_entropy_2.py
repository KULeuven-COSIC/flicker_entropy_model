...
data_storage = s_d.StoreData(name='comb_bit_entropy_2')
...
graph_maker = g_m.GraphMaker('comb_bit_entropy_2.svg', folder_name='figures')
...
for index, (t_acc, marker) in enumerate(zip(T_ACCS, T_ACC_MARKERS)):
    graph_maker.plot(ax=ax, xs=[], ys=[], line_style='none',
                     color=index, marker=marker,
                     label=(r"""$t_{acc} = \num{"""
                            f'{int(np.round(t_acc * 1e9)):d}' r"""} \; n s$"""), # type: ignore
                     visible=False)
