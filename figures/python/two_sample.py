"""Generate a figure showing a multivariate Gaussian PDF, for two samples."""
...
image_w0_f0_1: List[List[float]] = [[sum((sum((pdf_array_bit_1[i_w0][i_w1][i_f0][i_f1]
                                     for i_f1 in range(NB_POINTS))) * d_f1
                                     for i_w1 in range(NB_POINTS))) * d_w1
                                     for i_w0 in range(NB_POINTS)]
                                     for i_f0 in range(NB_POINTS)]
# image_w1_f1 = [[sum((sum((pdf_array[i_w0][i_w1][i_f0][i_f1]
#                           for i_f0 in range(NB_POINTS))) for i_w0 in range(NB_POINTS)))
#                           for i_w1 in range(NB_POINTS)] for i_f1 in range(NB_POINTS)]
image_w1_f1_1: List[List[float]] = [[sum((sum((pdf_array_bit_1[i_w0][i_w1][i_f0][i_f1]
...
#clip image:
image_w0_f0_1 = [[max(p, 1e-150) for p in row] for row in image_w0_f0_1]
image_w1_f1_1 = [[max(p, 1e-150) for p in row] for row in image_w1_f1_1]

graph_maker = g_m.GraphMaker('two_sample.svg', figure_size=(3, 3), folder_name='figures')
...
ax_w0_f0_c1 = graph_maker.create_ax(2, 0, show_x_labels=False, show_y_labels=False, # pylint: disable=invalid-name
                                    title=('$f_{\\Phi_e^w(t_1), \\Phi_e^f(t_1) \\mid B\'_1}'
                                           '(\\varphi^w, \\varphi^f \\mid 1)$'), title_pad=10)
ax_w1_f1_c1 = graph_maker.create_ax(8, 0, show_x_labels=False, show_y_labels=False, # pylint: disable=invalid-name
                                    title=('$f_{\\Phi_e^w(t_2), \\Phi_e^f(t_2) \\mid B\'_1}'
                                           '(\\varphi^w, \\varphi^f \\mid 1)$'), title_pad=10)
...
ax_w0_c1 = graph_maker.create_ax(2, 2, show_y_ticks=False, share_x=ax_w0_f0_c1, # pylint: disable=invalid-name
                                 x_scale='pi', max_nb_x_ticks=6,
                                 x_label='White phase ($\\varphi^w$)')
ax_f0_c1 = graph_maker.create_ax(0, 0, show_x_ticks=False, share_y=ax_w0_f0_c1, # pylint: disable=invalid-name
                                 y_scale='pi', max_nb_y_ticks=8,
                                 y_label='Flicker phase ($\\varphi^f$)')
ax_w1_c1 = graph_maker.create_ax(8, 2, show_y_ticks=False, share_x=ax_w1_f1_c1, # pylint: disable=invalid-name
                                 x_scale='pi', max_nb_x_ticks=6,
                                 x_label='White phase ($\\varphi^w$)')
ax_f1_c1 = graph_maker.create_ax(6, 0, show_x_ticks=False, share_y=ax_w1_f1_c1, # pylint: disable=invalid-name
                                 y_scale='pi', max_nb_y_ticks=8,
                                 y_label='Flicker phase ($\\varphi^f$)')

graph_maker.contour_fill(ax_w0_f0_c1, image_w0_f0_1, w_0s, f_0s,
                         color_norm=g_m.GraphMaker.ColorNorm.LOG,
                         norm_vmin=1e-6,
                         color_bar=ax_c_bar_0)
graph_maker.contour_fill(ax_w1_f1_c1, image_w1_f1_1, w_1s, f_1s,
                         color_norm=g_m.GraphMaker.ColorNorm.LOG,
                         norm_vmin=1e-12,
                         color_bar=ax_c_bar_1)

graph_maker.plot(ax_w0_c1, w_0s, pdf_w0_c1)
graph_maker.plot(ax_f0_c1, pdf_f0_c1, f_0s)
graph_maker.plot(ax_w1_c1, w_1s, pdf_w1_c1)
graph_maker.plot(ax_f1_c1, pdf_f1_c1, f_1s)
